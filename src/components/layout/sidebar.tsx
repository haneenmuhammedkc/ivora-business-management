...
export function Sidebar({ isOpen, onClose, className = "" }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 flex w-64 flex-col border-r border-gray-200/90 bg-white transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 ${
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={onClose}
                  className={`flex items-center gap-3.5 px-3 py-2.5 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                    isActive
        ...
        <div className="border-t border-gray-100 p-4">
          <div className="flex items-center justify-between rounded-lg p-2 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[#0c0d12] text-xs font-bold text-white tracking-wider">
                AV
              </div>
